...
main() async {
  ...
  group('Examples from https://formatjs.io/guides/message-syntax/', () {
    ...
    test('Simple Argument', () {
      expect(new IntlMessage('Hello {who}').format({'who': 'you'}), 'Hello you');
    });

    test('Number format', () {
      expect(new IntlMessage('I have {numCats, number} cats.').format({'numCats': 5}), 'I have 5 cats.');
      expect(new IntlMessage('Almost {pctBlack, number, percent} of them are black.').format({'pctBlack': 0.4}), 'Almost 40% of them are black.');
      expect(new IntlMessage('{value, number, integer}').format({'value': 5.5}), '6');
    });
    ...
    test('Date format', () {
      ...
      Intl.withLocale("nl-BE", () {
        expect(new IntlMessage('Solden beginnen {start, date, short}').format(args), 'Solden beginnen 3-11-2017');
        expect(new IntlMessage('Solden beginnen {start, date, medium}').format(args), 'Solden beginnen 3 nov. 2017');
        expect(new IntlMessage('Solden beginnen {start, date, long}').format(args), 'Solden beginnen 3 november 2017');
        expect(new IntlMessage('Solden beginnen {start, date, full}').format(args), 'Solden beginnen vrijdag 3 november 2017');
      });
      Intl.withLocale("en", () {
        expect(new IntlMessage('Sale begins {start, date, short}').format(args), 'Sale begins 11/3/2017');
        expect(new IntlMessage('Sale begins {start, date, medium}').format(args), 'Sale begins Nov 3, 2017');
        expect(new IntlMessage('Sale begins {start, date, long}').format(args), 'Sale begins November 3, 2017');
        expect(new IntlMessage('Sale begins {start, date, full}').format(args), 'Sale begins Friday, November 3, 2017');
      });
    ...
    test('Time format', () {
      ...
      expect(new IntlMessage('Coupon expires at {expires, time, short}').format(args), 'Coupon expires at 2:30 PM');
      expect(new IntlMessage('Coupon expires at {expires, time, medium}').format(args), 'Coupon expires at 2:30:00 PM');
    ...
    test('Custom format', () {
      ...
      Intl.withLocale("en_US", () {
        expect(new IntlMessage("Your total is {total, number, currency}").format({"total": 99}), r'Your total is $99.00');
        expect(new IntlMessage("Your total is {total, number, ¤#,##0.00}").format({"total": 99}), r'Your total is USD99.00');

        expect(new IntlMessage('Solden beginnen {start, date, EEEE}').format(args), 'Solden beginnen Friday');
        expect(new IntlMessage("Coupon expires at {start, time, h 'o''clock' a}").format(args), "Coupon expires at 2 o'clock PM");
      });

      Intl.withLocale("nl", () {
        expect(new IntlMessage("Your total is {total, number, currency}").format({"total": 99}), r'Your total is € 99,00');
        expect(new IntlMessage("Your total is {total, number, ¤#,##0.00}").format({"total": 99}), r'Your total is EUR99,00');
    ...
    test('Gender', () {
      ...
      var m = new IntlMessage("{gender, select, male {He} female {She} other {They}} will respond shortly.");
    ...
    test('Nested select', () {
      ...
      expect(m.format({"taxableArea": "yes", "taxRate": 0.3}), "An additional 30% tax will be collected.");
      expect(m.format({"taxableArea": "no", "taxRate": 0.3}), "No taxes apply.");
    });

    test('Plural', () {
      var m = new IntlMessage("Cart: {itemCount} {itemCount, plural, one {item} other {items}}");

      expect(m.format({"itemCount": 1}), "Cart: 1 item");
      expect(m.format({"itemCount": 2}), "Cart: 2 items");
      expect(m.format({"itemCount": 0}), "Cart: 0 items");

      m = new IntlMessage("You have {itemCount, plural, =0 {no items} one {1 item} other {{itemCount} items}}.");
      ...
      m = new IntlMessage("You have {itemCount, plural, =0 {no items} one {# item} other {# items}}.");
    ...
    test('Selectordinal', () {
      ...
      Intl.withLocale("af", () {
        expect(new IntlMessage("Neem die {number, selectordinal, one {ERROR} other {#e}} afdraai na regs.").format({"number": 1}), "Neem die 1e afdraai na regs.");
      });
      var m = new IntlMessage("It's my cat's {year, selectordinal, one {#st} two {#nd} few {#rd} other {#th}} birthday!");
  ...
  group('examples from https://messageformat.github.io/guide/', () {
    ...
    test('offset extension', () {
      ...
      expect(
          m.format({ "NUM_ADDS": 2}), "You and one other person added this.");
    ...
    test('date', () {
      ...
      expect(new IntlMessage('Unix time started on {T, date, full}')
          .format({"T": new DateTime(1970)}),
          'Unix time started on Thursday, January 1, 1970');
      expect(new IntlMessage('{sys} became operational on {d0, date, short}')
    ...
    test('number', () {
      expect(
          new IntlMessage('{N} is almost {N, number, integer}').format({"N": 3.14}),
          '3.14 is almost 3');
      expect(new IntlMessage('{P, number, percent} complete').format({"P": 0.99}),
          '99% complete');

      IntlMessage.withCurrency("EUR", () {
        expect(
            new IntlMessage('The total is {V, number, currency}.').format({"V": 5.5}),
            'The total is €5.50.');
      });
    });
    ...
    test('time', () {
      ...
      expect(
          new IntlMessage('The time is now {T, time}').format({"T": now}),
          'The time is now 11:26:35 PM');

      Intl.withLocale("fi", () {
        expect(
            new IntlMessage('Kello on nyt {T, time}').format({"T": now}),
    ...
    test('custom formatters', () {
      ...
      IntlMessage.withFormatters({
        ...
        expect(
            new IntlMessage('This is {VAR, upcase}.').format({"VAR": 'big'}),
            'This is BIG.');
        ...
        expect(
            new IntlMessage('Answer: {obj, prop, a}').format({"obj": {"q": 3, "a": 42}}),
            'Answer: 42');
      });
  ...
  group('and changing the locale',  () {
    ...
    test('should format message en-US complex with different objects',  () {
      ...
      expect(msgFmt.format(maleTravelers),'Lucas, Tony and Drew went to Paris.');
    ...
    test('should format message fr-FR complex with different objects',  () {
      Intl.withLocale("fr-FR", () {
        ...
        expect(msgFmt.format(maleTravelers),'Lucas, Tony and Drew sont allés à Paris.');
  ...
  group('and change the locale with different counts',  () {
    ...
    test('should format a message with en-US locale',  () {
      ...
      expect(msgFmt.format({"COMPANY_COUNT": 0}),'0 companies published new books.');
      expect(msgFmt.format({"COMPANY_COUNT": 1}),'One company published new books.');
      expect(msgFmt.format({"COMPANY_COUNT": 2}),'2 companies published new books.');
      expect(msgFmt.format({"COMPANY_COUNT": 5}),'5 companies published new books.');
      expect(msgFmt.format({"COMPANY_COUNT": 10}),'10 companies published new books.');
    });
    ...
    test('should format a message with ru-RU locale',  () {
      Intl.withLocale("ru-RU", () {
        ...
        expect(msgFmt.format({"COMPANY_COUNT": 0}),'0 компаний опубликовали новые книги.');
        expect(msgFmt.format({"COMPANY_COUNT": 1}),'Одна компания опубликовала новые книги.');
        expect(msgFmt.format({"COMPANY_COUNT": 2}),'2 компании опубликовали новые книги.');
        expect(msgFmt.format({"COMPANY_COUNT": 5}),'5 компаний опубликовали новые книги.');
        expect(msgFmt.format({"COMPANY_COUNT": 10}),'10 компаний опубликовали новые книги.');
        expect(msgFmt.format({"COMPANY_COUNT": 21}),'21 компания опубликовала новые книги.');
      });
    });
  });
  ...
  group('arguments with',  () {
    ...
    group('no spaces', () {
      var msg   = new IntlMessage('{STATE}'),
          state = 'Missouri';

      test('should fail when the argument in the pattern is not provided',  () {
        expect(()=>msg.format({}), throwsA(new isInstanceOf<ArgumentError>()));
      });

      test('should fail when the argument in the pattern has a typo',  () {
        expect(()=>msg.format({'ST ATE': state}), throwsA(new isInstanceOf<ArgumentError>()));
      });
    ...
    group('a numeral', () {
      var msg   = new IntlMessage('{ST1ATE}'),
          state = 'Missouri';

      test('should fail when the argument in the pattern is not provided',  () {
        expect(()=>msg.format({'FOO': state}), throwsA(new isInstanceOf<ArgumentError>()));
      });
  ...
  group('subindex', () {
    test('', () {
      ...
      expect(msg.format({"person": {"firstname": "Rik"}}), "Rik");
