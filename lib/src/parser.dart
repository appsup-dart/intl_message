...
class IcuParser {
  ...
  Parser get styleStart => string("<style")&(whitespace()&char(">").neg().star()).optional()&char(">");
  ...
  Parser get messageText => (icuEscapedText | icuText | style).plus().flatten().map((v)=>new LiteralString(v));
  Parser get nonIcuMessageText => normalText.plus().flatten().map((v)=>new LiteralString(v));
  Parser get twoSingleQuotes => string("''").map((x) => "'");
  Parser get number => digit().plus().flatten().trim().map(int.parse);
  Parser get simpleId => ((letter()|char("_")) & (word() | char("_")).star()).flatten();
  Parser get id => simpleId.map((v)=>new Variable(v)).seq(char(".").seq(simpleId).pick(1).star())
      .map((v)=>(v[1] as List).fold(v[0],(a,b)=>a.subIndex(b))).trim();
  ...
  Parser asKeywords(list) => list.map(string).reduce((a, b) => a | b).flatten().trim();

  Parser get pluralKeyword => asKeywords(["zero", "one", "two", "few", "many", "other"])
  ...
  Parser get intlNumber =>
      (preface & numberLiteral & (comma & icuText.plus().flatten().trim()).pick(1).optional() & closeCurly)
  ...
  Parser get intlDate =>
      (preface & dateLiteral & (comma & dateFormat).pick(1).optional() & closeCurly)
  ...
  Parser get intlTime =>
      (preface & timeLiteral & (comma & timeFormat).pick(1).optional() & closeCurly)
  ...
  Parser get intlPlural =>
      plural.map((values) => values[1]=="plural" ?
      new PluralMessage(values.first, values[4], offset: values[3] ?? 0) :
      new SelectOrdinalMessage(values.first, values[4], offset: values[3] ?? 0));
  ...
  Parser get intlSelect => generalSelect
      .map((values) => new SelectMessage(values.first, values[3]));
